...
export function resolveShell(options: ResolveShellOptions = {}) {
	const detect = options.detect ?? detectDefaultShell;
	const defaultShellPath =
		options.defaultShellPath === undefined ? defaultShell : options.defaultShellPath;
	const env = options.env ?? process.env;

	try {
		return detect();
	} catch {
		if (defaultShellPath) return defaultShellPath;
		return env.SHELL || '/bin/sh';
	}
}

export function resolveShellArgs(shellPath: string) {
...
export function updateFocusReportingState(
	session: Pick<TerminalSession, 'focusReportingEnabled' | 'modeSequenceTail'>,
	chunk: string,
) {
	const combined = session.modeSequenceTail + chunk;
	// biome-ignore lint/suspicious/noControlCharactersInRegex: ESC (\x1b) is required to match ANSI focus-reporting mode toggles.
...
export function signalTerminalProcessGroup(
