...
type ParsedArgs = { kind: 'run'; options: CliOptions } | { kind: 'help' } | { kind: 'version' };
...
async function maybeSelfUpdate(_argv: string[], deps: CliRuntimeDeps) {
	if (process.env.MIKO_DISABLE_SELF_UPDATE === '1') {
		return null;
	}
...
export async function runCli(argv: string[], deps: CliRuntimeDeps): Promise<CliRunResult> {
	...
	const displayHost =
		parsedArgs.options.share || bindHost === '127.0.0.1' || bindHost === '0.0.0.0'
			? 'localhost'
			: bindHost;
	...
	if (parsedArgs.options.share) {
		try {
			const shareTunnel = await (
				deps.startShareTunnel ??
				((localUrl) =>
					startShareTunnel(localUrl, {
			...
			await logShareDetails(
				deps.log,
				shareTunnel.publicUrl,
				launchUrl,
				deps.renderShareQr ?? renderTerminalQr,
			);
		} catch (error) {
			await stop();
