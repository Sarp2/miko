...
export async function ensureCloudflaredInstalled(deps: ShareTunnelDeps = {}) {
	...
	const publicUrl = await new Promise<string>((resolve, reject) => {
		...
		const handleExit = (code: number | null, signal: NodeJS.Signals | null) => {
			settle(() =>
				reject(
					new Error(
						`Cloudflare tunnel exited before a public URL was ready (code: ${String(code)}, signal: ${String(signal)})`,
					),
				),
			);
		};
