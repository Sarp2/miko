...
export function normalizeExistingDirectory(localPath: string) {
	try {
...
export function readCodexSessionIndex(indexPath: string) {
	...
	for (const line of readFileSync(indexPath, 'utf8').split('\n')) {
		...
		const updatedAt =
			typeof record.updated_at === 'string' ? Date.parse(record.updated_at) : Number.NaN;
...
export function collectCodexSessionFiles(directory: string): string[] {
...
export function readCodexConfiguredProjects(configPath: string) {
...
export function readCodexSessionMetadata(sessionsDir: string) {
	...
	for (const sessionFile of collectCodexSessionFiles(sessionsDir)) {
		...
		const payloadTimestamp =
			typeof payloadRecord.timestamp === 'string'
				? Date.parse(payloadRecord.timestamp)
				: Number.NaN;

		const modifiedAt =
			[recordTimestamp, payloadTimestamp, fileStat.mtimeMs].find((value) => !Number.isNaN(value)) ??
			fileStat.mtimeMs;
...
export function discoverProjects(
	...
	const mergedProjects = mergeDiscoveredProjects(
		adapters.flatMap((adapter) =>
			adapter.scan(homeDir).map(({ provider: _provider, ...project }) => project),
		),
	);
	return mergedProjects;
}
