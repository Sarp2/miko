...
export function structuredOutputFromSdkMessage(message: unknown): unknown | null {
...
export class QuickResponseAdapter {
	...
	constructor(args: QuickResponseAdapterArgs = {}) {
		...
		this.runCodexStructured =
			args.runCodexStructured ??
	...
	async generateStructuredWithDiagnostics<T>(
		...
		const codexResult = await this.tryProvider('codex', args.task, args.parse, () =>
			this.runCodexStructured(request),
		);
