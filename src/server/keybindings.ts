...
export function normalizeKeybindings(
	value: KeybindingsFile | null | undefined,
	filePath = getKeybindingsFilePath(homedir()),
): KeybindingsSnapshot {
	...
	if (!source) {
		return createDefaultSnapshot(
			filePath,
			'Keybindings file must contain a JSON object. Using defaults.',
		);
	}
	...
	return {
		...
		warning:
			warnings.length > 0
				? `Some keybindings were reset to defaults: ${warnings.join('; ')}`
				: null,
