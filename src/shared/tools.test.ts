...
import { hydrateToolResult, normalizeToolCall } from 'src/shared/tools';
...
describe('hydrateToolResult', () => {
	...
	test('hydrates AskUserQuestion multi-select answers', () => {
		...
		const result = hydrateToolResult(
			tool,
			JSON.stringify({ answers: { runtime: ['bun', 'node'] } }),
		);
		expect(result).toEqual({ answers: { runtime: ['bun', 'node'] } });
	});

	test('hydrates ExitPlanMode', () => {
		const tool = normalizeToolCall({
