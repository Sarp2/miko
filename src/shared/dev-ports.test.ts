...
describe('resolveDevPorts', () => {
	...
	test('uses the last provided --port value', () => {
		expect(resolveDevPorts(['--remote', '4000', '--port', '4100'])).toEqual({
			clientPort: 4100,
			serverPort: 4101,
		});
	});
...
describe('parseDevArgs', () => {
	...
	test('rejects combining --share with --host or --remote', () => {
		expect(() => parseDevArgs(['--share', '--host', 'dev-box'], 'dev-machine')).toThrow(
			'--share cannot be used with --host',
		);

		expect(() => parseDevArgs(['--host', 'dev-box', '--share'], 'dev-machine')).toThrow(
			'--share cannot be used with --host',
		);
	...
	test('preserves existing remote host behavior when share is off', () => {
		expect(parseDevArgs(['--remote', '--port', '3333'], 'dev-maching')).toEqual({
			clientPort: 3333,
			serverPort: 3334,
			share: false,
			backendTargetHost: '127.0.0.1',
